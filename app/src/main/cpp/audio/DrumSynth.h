...
private:
  ...
  struct KickState {
    ...
    float pitchEnv = 0.0f;
    float ampEnv = 0.0f;
    float velocity = 1.0f; // Added velocity
    int sampleCount = 0;
    ...
    static constexpr float AMP_DECAY =
        0.00005f; // Medium decay - punchy but present
  ...
  // ========== HI-HAT ==========
  // 707-style closed hi-hat: 6 square wave oscillators at inharmonic
  // frequencies Creates metallic, shimmering tone characteristic of analog drum
  // machines
  struct HiHatState {
    bool active = false;
    ...
    static constexpr float AMP_DECAY =
        0.0006f; // Faster decay for dead/tight hi-hat
