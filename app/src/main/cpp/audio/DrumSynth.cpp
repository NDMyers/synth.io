...
void DrumSynth::setSampleRate(float sampleRate) {
    mSampleRate = sampleRate;
}

void DrumSynth::triggerKick() {
    mKick.active = true;
    mKick.phase = 0.0f;
    mKick.pitchEnv = 1.0f;
    mKick.ampEnv = 1.0f;
...
void DrumSynth::triggerHiHat(float velocity) {
    mHiHat.active = true;
    mHiHat.velocity = std::max(0.3f, std::min(1.0f, velocity));  // Clamp with minimum
    mHiHat.ampEnv = 1.0f;
...
float DrumSynth::generateKickSample() {
    ...
    float currentFreq = KickState::END_FREQ + 
    ...
    // Tiny click at start
    float clickDurationSamples = (KickState::CLICK_DURATION_MS / 1000.0f) * mSampleRate;
    ...
    if (mKick.phase >= 1.0f) mKick.phase -= 1.0f;
    ...
    return sample * 1.0f;  // Clean unity output
}
...
float DrumSynth::generateSnareSample() {
    ...
    float f = 2.0f * std::sin(static_cast<float>(M_PI) * SnareState::BP_FREQ / mSampleRate);
    ...
    if (mSnare.bodyPhase >= 1.0f) mSnare.bodyPhase -= 1.0f;
...
float DrumSynth::generateHiHatSample() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (mHiHat.phases[i] >= 1.0f) mHiHat.phases[i] -= 1.0f;
    }
    ...
    float hpCoeff = 1.0f - std::exp(-2.0f * static_cast<float>(M_PI) * HiHatState::HP_FREQ / mSampleRate);
    ...
    // Mix tone and noise
    float sample = filteredTone * HiHatState::TONE_MIX + filteredNoise * HiHatState::NOISE_MIX;
