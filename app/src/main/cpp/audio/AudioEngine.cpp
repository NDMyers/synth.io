...
struct TrueLimiter {
    ...
    float releaseCoeff = 0.0001f; // Slow release to avoid pumping (~50ms at 48kHz)
...
void AudioEngine::onErrorAfterClose(oboe::AudioStream *audioStream, oboe::Result error) {
    ...
    LOGI("Audio stream disconnected (error: %s), restarting...", oboe::convertToText(error));
...
oboe::Result AudioEngine::createStream() {
    ...
    builder.setDirection(oboe::Direction::Output)
           ...
           ->setSharingMode(oboe::SharingMode::Shared)  // Shared mode for Bluetooth compatibility
    ...
    if (result == oboe::Result::OK && mStream) {
        ...
        LOGI("  Performance mode: %s",
             mStream->getPerformanceMode() == oboe::PerformanceMode::LowLatency ? "LowLatency" : "Other");
        ...
        if (desiredBuffer < bufferSize) {
            ...
            LOGI("  Reduced buffer to: %d frames (%.1f ms)", 
                 desiredBuffer, (desiredBuffer * 1000.0) / sampleRate);
        }
    }
...
void AudioEngine::looperStartRecording() {
    ...
    LOGI("Looper: Starting recording (pre-count) with metronome at %.1f BPM", bpm);
}
...
void AudioEngine::looperStartPlayback() {
    ...
    LOGI("Looper: Starting playback (drums %s)", mDrumEnabledByUser ? "synced" : "off");
}
...
bool AudioEngine::looperHasLoop() const {
    return mLooper.hasLoop();
}
...
int AudioEngine::getLooperCurrentBar() const {
    return mLooper.getCurrentBar();
}
...
void AudioEngine::looperStartRecordingTrack(int trackIndex) {
    ...
    LOGI("Looper: Starting recording track %d with metronome at %.1f BPM", trackIndex, bpm);
}
    ...
    for (int i = 0; i < numFrames; ++i) {
        ...
        if (needsMetronome) {
            ...
            if (!metronomeWasActive) {
                ...
                // Trigger first snare immediately (higher pitch than kick, cuts through better)
                mDrumMachine.triggerSnare();
                LOGI("Metronome started via DrumMachine snare, BPM=%.1f", mDrumMachine.getBPM());
            }
            
            // Get the drum synth output - use higher volume for metronome
        ...
        //
        constexpr float SYNTH_GAIN = 0.045f; // Synth reduced (raw signal is very hot)
        ...
        float metroMix = metronomeSample * METRO_GAIN;
