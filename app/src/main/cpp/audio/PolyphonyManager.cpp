...
void PolyphonyManager::noteOff(int midiNote) {
    ...
    for (auto& voice : mVoices) {
        if (voice.getMidiNote() == midiNote && voice.getState() == VoiceState::ACTIVE) {
            voice.noteOff();
        }
    }
}
...
// ===== LFO PARAMETERS =====
void PolyphonyManager::setLFORate(float rateHz) {
    mLFO.setRate(rateHz);
}
...
void PolyphonyManager::setLFOPWMDepth(float depth) {
    mLFO.setPWMDepth(depth);
}
...
void PolyphonyManager::noteOffUnison(int midiNote) {
    ...
    for (int i = 0; i < MAX_POLYPHONY; ++i) {
        if (mVoices[i].getMidiNote() == midiNote && mVoices[i].getState() == VoiceState::ACTIVE) {
            mVoices[i].noteOff();
            mUnisonNoteVoices[i] = -1;
        }
    }
...
float PolyphonyManager::softLimit(float sample) {
    ...
    if (absSample <= threshold) {
        ...
        float limited = threshold + knee * 0.5f + (1.0f - threshold - knee * 0.5f) * std::tanh(excess * 2.0f);
...
void PolyphonyManager::applyParamsToVoice(Voice& voice) {
    voice.setWaveform(mCurrentWaveform);
